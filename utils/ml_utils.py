...
def utils_recognize_type(dtf, col, max_cat=20):
    ...
    Args:
        dtf: dataframe - input data
        col: str - name of the column to analyze
        max_cat: num - max number of unique values to recognize a column as categorical
    Return:
...
def pop_columns(dtf, lst_cols, where="front"):
    ...
    Args:
        dtf: dataframe - input data
        lst_cols: list - names of the columns that must be moved
        where: str - "front" or "end"
    Returns:
        dtf with moved columns
    """
...
def freqdist_plot(dtf, x, max_cat=20, top=None, show_perc=True, bins=100, quantile_breaks=(0, 10), box_logscale=False,
                  figsize=(10, 5), with_normal=False):
    """
    Plots the frequency distribution of a dtf column.
    Args:
        dtf: dataframe - input data
        x: str - column name
        max_cat: num - max number of uniques to consider a numerical variable as categorical
        top: num - plot setting
        show_perc: logic - plot setting
        bins: num - plot setting
        quantile_breaks: tuple - plot distribution between these quantiles (to exclude outilers)
        box_logscale: logic
        figsize: tuple - plot settings
    """
    try:
        ...
        ## num --> density
            ...
            if with_normal:
                x = np.linspace(int(float(des['min'])), int(float(des['max'])), max(10000,int(float(des['max']) - float(des['min']))))
                y = norm.pdf(x, loc=float(des['mean']), scale=float(des['std']))
                ax[0].plot(x, y, color="red", label='Normal distribution')
                ax[0].legend()
            plt.show()
    ...
    try:
        ...
        ## num vs cat --> density + stacked + boxplot 
            ...
            for i in sorted(dtf[cat].unique()):
                sns.distplot(dtf[dtf[cat] == i][num], hist=False, label=i, ax=ax[0], norm_hist=True)
            ax[0].grid(True)
            ax[0].legend()
...
def corr_matrix(dtf, method="pearson", negative=True, lst_filters=[], annotation=True, figsize=(10, 5)):
    ...
    Args:
        dtf: dataframe - input data
        method: str - "pearson" (numeric), "spearman" (categorical), "kendall"
        negative: bool - if False it takes the absolute values of correlation
        lst_filters: list - filter rows to show
        annotation: logic - plot setting
    """
...
def rebalance(dtf, y, balance=None, method="random", replace=True, size=1):
    ...
    Args:
        dtf: dataframe - feature matrix dtf
        y: str - column to use as target
        balance: str - "up", "down", if None just prints some stats
        method: str - "random" for sklearn or "knn" for imblearn
        size: num - 1 for same size of the other class, 0.5 for half of the other class
    Returns:
        rebalanced dtf
    """
    ## check
...
def add_feature_clusters(dtf, x, dic_clusters_mapping, dropx=False):
    ...
    Args:
        dtf: dataframe - feature matrix dtf
        x: str - column name
        dic_clusters_mapping: dict - ex: {"min":[30,45,180], "max":[60,120], "mean":[]}  where the residual class must have an empty list
        dropx: logic - whether the x column should be dropped
    """
...
def data_preprocessing(dtf, y, processNas=None, processCategorical=None, split=None, scale=None, task="classification"):
    ...
    Args:
        ...
        processNas: str or None - "mean", "median", "most_frequent"
        processCategorical: str or None - "dummies"
        split: num or None - test_size (example 0.2)
        scale: str or None - "standard", "minmax"
...
def features_selection(dtf, y, top=10, task="classification", figsize=(20, 10)):
    ...
    Args:
        dtf: dataframe - feature matrix dtf
        y: str - name of the dependent variable
        top: num - number of top features
        task: str - "classification" or "regression"
    Returns:
    ...
    try:
        ...
        model = linear_model.LogisticRegression(C=1, penalty="l1",
                                                solver='liblinear') if task == "classification" else linear_model.Ridge(
        ...
        dtf_features["method"] = dtf_features["method"].apply(
            lambda x: x.split()[0] + " + " + x.split()[1] if len(x.split()) == 2 else x)
        fig, ax = plt.subplots(figsize=figsize)
        sns.barplot(y="features", x="selection", hue="method",
                    data=dtf_features.sort_values("selection", ascending=False), ax=ax, dodge=False)

        join_selected_features = list(set(pvalue_selected_features).intersection(regularization_selected_features))
        return {"p_value": pvalue_selected_features, "regularization": regularization_selected_features,
                "join": join_selected_features}
...
def features_importance(X, y, X_names, model=None, task="classification", figsize=(10, 10)):
    ...
    Args:
        X: array
        X_names: list
        model: model istance (after fitting)
...
def fit_ml_classif(model, X_train, y_train, X_test, threshold=0.5):
    ...
    Args:
        ...
        y_train: array
        X_test: array
        threshold: num - predictions > threshold are 1, otherwise 0 (only for classification)
    Returns:
...
def utils_threshold_selection(model, X, y, figsize=(10, 5)):
    ...
    predicted_prob = model.fit(XX_train, yy_train).predict_proba(XX_test)[:,
                     1]  # if "tensorflow" not in type(model) else model.fit(XX_train, yy_train, batch_size=32, epochs=100, verbose=0).model.predict(XX_test)
    thresholds = []
...
def tune_classif_model(X_train, y_train, model_base=None, param_dic=None, scoring="f1", searchtype="RandomSearch",
    ...
    Args:
        model_base: model object - model istance to tune (before fitting)
        param_dic: dict - dictionary of parameters to tune
        X_train: array - feature matrix
        y_train: array - y vector
        scoring: string - "roc_auc", "accuracy", "f1", "precision", "recall"
        searchtype: string - "RandomSearch" or "GridSearch"
    Returns:
...
def fit_dl_classif(X_train, y_train, X_test, model=None, batch_size=32, epochs=100, threshold=0.5):
    ...
    Args:
        ...
        y_train: array
        X_test: array
        batch_size: num - keras batch
        epochs: num - keras epochs
        threshold: num - predictions > threshold are 1, otherwise 0
    Returns:
        model fitted and predictions
    """
    ## model
...
def tune_regr_model(X_train, y_train, model_base=None, param_dic=None, scoring="r2", searchtype="RandomSearch",
                    n_iter=1000, cv=10, figsize=(10, 5)):
    ...
    print(scoring, "mean - base model:", round(Kfold_base["test_score"].mean(), 2), " --> best model:",
          round(Kfold_model["test_score"].mean()))
...
def fit_dl_regr(X_train, y_train, X_test, scalerY, model=None, batch_size=32, epochs=100):
    ...
    ## train
    verbose = 0 if epochs > 1 else 1
    training = model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs, shuffle=True, verbose=verbose,
                         validation_split=0.3)
    if epochs > 1:
        utils_plot_keras_training(training)
...
def explainer_shap(model, X_names, X_instance, X_train=None, task="classification", top=10):
    ...
    ### regression
        ...
        shap.waterfall_plot(explainer.expected_value[0], shap_values,
            feature_names=X_names, max_display=top)
...
def explainer_lime(X_train, X_names, model, y_train, X_instance, task="classification", top=10):
    if task == "classification":
        explainer = lime_tabular.LimeTabularExplainer(training_data=X_train, feature_names=X_names,
                                                      class_names=np.unique(y_train), mode=task)
        ...
        explainer = lime_tabular.LimeTabularExplainer(training_data=X_train, feature_names=X_names, class_names="Y",
                                                      mode=task)
        ...
        for m in range(n_a):
            for o in range(n_b):
                line = plt.Line2D([i * x_space + left, (i + 1) * x_space + left],
                                  [layer_top_a - m * y_space, layer_top_b - o * y_space], c='k', alpha=0.5)
                ax.add_artist(line)
    plt.show()
...
def fit_dl_cluster(X, model=None, epochs=100, lst_2Dplot=None, figsize=(10, 5)):
    ## model
    model = minisom.MiniSom(x=int(np.sqrt(5 * np.sqrt(X.shape[0]))), y=int(np.sqrt(5 * np.sqrt(X.shape[0]))),
                            input_len=X.shape[1]) if model is None else model
    ...
    if (lst_2Dplot is not None) or (X.shape[1] == 2):
        lst_2Dplot = X.columns.tolist() if lst_2Dplot is None else lst_2Dplot
        utils_plot_cluster(dtf_X, x1=lst_2Dplot[0], x2=lst_2Dplot[1],
                           th_centroids=scaler.inverse_transform(cluster_centers), figsize=figsize)
...
def plot_map(dtf, x, y, start, zoom=12, tiles="cartodbpositron", popup=None, size=None, color=None, legend=False,
    ...
    if marker is not None:
        ...
        ### binary case (1/0): mark only 1s
            ...
            data[data[marker] == lst_elements[1]].apply(
                lambda row: folium.Marker(location=[row[x], row[y]], popup=row[marker], draggable=False,
            ...
            for i in lst_elements:
                data[data[marker] == i].apply(
                    lambda row: folium.Marker(location=[row[x], row[y]], popup=row[marker], draggable=False,
                                              icon=folium.Icon(color=lst_colors[lst_elements.index(i)])).add_to(map_),
                    axis=1)
    return map_
